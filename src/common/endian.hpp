...
// TODO: use static reflection to generate byteswap function?
// but reflection support across compilers is pretty spotty atm
template <typename T>
concept NonTrivialByteSwappable = requires (const T& v) {
    { v.byteswap() } -> std::same_as<T>;
};

template <typename T>
concept NonTrivialInplaceByteSwappable = requires (T& v) {
    { v.byteswap() } -> std::same_as<void>;
};
...
template <NonTrivialByteSwappable T>
constexpr auto ByteSwap(const T& value) -> T {
...
template <typename T>
requires std::integral<T> || std::floating_point<T> || NonTrivialInplaceByteSwappable<T>
...
template <NonTrivialInplaceByteSwappable T>
constexpr auto InplaceByteSwap(T& value) -> void {
    value.byteswap();
}
