...
auto CalcMmh3(const std::uint8_t* data, size_t size, std::uint32_t seed) -> std::uint32_t {
    std::uint32_t hash;
    MurmurHash3_x86_32(data, static_cast<std::int32_t>(size), seed, &hash);
    return hash;
}

auto CalcMmh3(const char* str) -> std::uint32_t {
    return CalcMmh3(reinterpret_cast<const std::uint8_t*>(str), std::strlen(str));
}

auto CalcMmh3(const std::string_view str) -> std::uint32_t {
    return CalcMmh3(reinterpret_cast<const std::uint8_t*>(str.data()), str.size());
}
