...
auto main(int argc, const char** argv) -> int {
    ...
    // if (argc < 2) {
    //     std::println("Please provide an input filepath");
    //     return 1;
    // }

    // const auto inputPath = std::string(argv[1]);
    // auto archiveRes = decode::BarsReader::Read(inputPath);
    // if (!archiveRes) {
    //     std::println("Failed to parse BARS: {} {:#x}", decode::ToString(archiveRes.error()), static_cast<std::uint32_t>(archiveRes.error()));
    //     return 1;
    // }

    // auto& archive = *archiveRes;
    // encode::BarsWriter::Write(*archive, "out.bars");
    // std::print("{}", archive->getAssets().front().dumpMetadata());
    ...
    //     auto soundRes = decode::BwavReader::Read(inputPath);
    //     if (!soundRes) {
    //         std::println("Failed to parse BWAV: {}", static_cast<std::uint32_t>(soundRes.error()));
    //         return 1;
    //     }
    //     sound = std::move(*soundRes);
    // } else {
    //     auto soundRes = decode::WavReader().open(inputPath);
    //     if (!soundRes) {
    //         std::println("Failed to parse WAV: {}", static_cast<std::uint32_t>(soundRes.error()));
    //         return 1;
    //     }
    //     sound = std::move(*soundRes);
    // }
    
    // sound->resampleAndConvert(sound::Format::DspAdpcm, 48000);
    // auto writer = encode::BwavWriter();
    // encode::BwavWrite::Write(*sound, "out.bwav");

    // auto source = playback::SoundSource();
    // if (!source.initialize(*sound)) {
    //     std::println("Failed to initialize sound source");
    //     return 1;
    // }

    // if (!source.play()) {
    //     std::println("Failed to start playback");
    //     return 1;
    // }
    
    // std::println("Currently playing {} ({} sec)", argv[1], sound->getDuration());
    // std::println("Press S to stop");
    // for (std::int32_t ch; (ch = std::getchar()) != EOF;) {
    //     if (std::isspace(static_cast<char>(ch))) {
    //         continue;
