...
uint32_t dsptool_getBytesForAdpcmBuffer(uint32_t samples)
{
...
uint32_t dsptool_getBytesForAdpcmInfo(void)
{
	return sizeof(ADPCMINFO);
}

uint32_t dsptool_getBytesForAdpcmSamples(uint32_t samples)
{
...
uint32_t dsptool_getBytesForPcmBuffer(uint32_t samples)
{
	uint32_t frames = samples / SAMPLES_PER_FRAME;
	if (samples % SAMPLES_PER_FRAME)
		frames++;
...
uint32_t dsptool_getBytesForPcmSamples(uint32_t samples)
{
	return samples * sizeof(int16_t);
}

uint32_t dsptool_getNibbleAddress(uint32_t samples)
{
...
uint32_t dsptool_getNibblesForNSamples(uint32_t samples)
{
	uint32_t frames = samples / SAMPLES_PER_FRAME;
	uint32_t extraSamples = samples % SAMPLES_PER_FRAME;
...
uint32_t dsptool_getSampleForAdpcmNibble(uint32_t nibble)
{
