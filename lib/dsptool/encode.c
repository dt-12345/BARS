...
void dsptool_correlateCoefs(int16_t* source, uint32_t samples, int16_t* coefsOut);
void DSPEncodeFrame(short pcmInOut[16], int sampleCount, unsigned char adpcmOut[8], const short coefsIn[8][2]);

void dsptool_encode(int16_t* src, uint8_t* dst, ADPCMINFO* cxt, uint32_t samples, uint8_t loop)
{
	int16_t* coefs = cxt->coef;
	dsptool_correlateCoefs(src, samples, coefs);
	...
	for (int i = 0; i < frameCount; ++i, pcm += SAMPLES_PER_FRAME, adpcm += BYTES_PER_FRAME)
	{
		...
		// nintendo aligns to a frame for non-looping audio
		if (i + 1 == frameCount && loop == 0) {
			memcpy(adpcm, adpcmFrame, dsptool_getBytesForAdpcmBuffer(sampleCount));
		} else {
			memcpy(adpcm, adpcmFrame, dsptool_getBytesForAdpcmSamples(sampleCount));
		}
	}
...
void dsptool_encodeFrame(int16_t* src, uint8_t* dst, int16_t* coefs, uint8_t one)
{
    (void)one;
